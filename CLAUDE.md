...
### CLI引数パース（`moonbitlang/core/argparse`）

CLI引数パースは公式の `@argparse` ライブラリで統一。手動パースは禁止。

- 各CLIモジュールは `command() -> @argparse.Command` でコマンド定義を公開
- `run_matches(matches : @argparse.Matches)` でハンドラを実装
- `Config::from_matches(matches)` で `Matches` → 設定構造体へ変換
- ヘルプは `@argparse` が自動生成（手動の `show_help()` は禁止）
...
**ドッグフーディング中にindexionコマンドが期待通りに動かなかった場合、代替手段で目的を達成してはならない。**

その問題をまず修正せよ。これがドッグフーディングの本質である。

- indexionのコマンドで検索して見つからなかった → **なぜ見つからなかったのかを調査し、indexion側を修正する**
- indexionの出力が不完全・不正確だった → **その場でindexionのコードを修正する**
- 「とりあえずgrepで」「とりあえずReadで」と別の手段に逃げることは、問題の先送りであり、ドッグフーディングの放棄である

問題を見つけること自体は成果だが、**見つけた問題を修正せずに先に進むことは許されない。**
