...
export const FileItem = ({ file, depth, onSelectSym }: Props): React.JSX.Element => {
    ...
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild>
        ...
        >
          ...
          <span className="font-mono text-sm text-muted-foreground">{file.name}</span>
          {file.symbols.length > 0 && (
            <Badge variant="outline" className="ml-auto text-[10px]">{file.symbols.length}</Badge>
          )}
        </button>
      </CollapsibleTrigger>
      {file.symbols.length > 0 && (
        <CollapsibleContent>
          {file.symbols.map((sym) => (
            ...
            >
              <Badge variant="outline" className={cn("font-mono text-[10px]", kindBadgeClass(sym.kind))}>
