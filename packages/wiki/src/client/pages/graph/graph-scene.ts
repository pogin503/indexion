...
const greedyColor = (nodeIds: string[], adjacency: Map<string, Set<string>>): Map<string, number> => {
  const colors = new Map<string, number>();
  for (const id of nodeIds) {
    const used = new Set<number>();
    const adj = adjacency.get(id);
    if (adj) for (const nb of adj) { const c = colors.get(nb); if (c !== undefined) used.add(c); }
    let c = 0;
    while (used.has(c)) c++;
    colors.set(id, c);
  }
...
export const buildScene = (
  ...
  for (const [id, node] of nodes) {
    ensureAdj(id);
    for (const child of node.children) { ensureAdj(child); adjacency.get(id)!.add(child); adjacency.get(child)!.add(id); }
  }
  for (const edge of edges) { ensureAdj(edge.from); ensureAdj(edge.to); adjacency.get(edge.from)!.add(edge.to); adjacency.get(edge.to)!.add(edge.from); }
  const nodeColorMap = greedyColor([...nodes.keys()], adjacency);
  const getNodeColor = (id: string): number => GRAPH_COLORS[(nodeColorMap.get(id) ?? 0) % GRAPH_COLORS.length];
  ...
  const texCvs = document.createElement("canvas"); texCvs.width = 512; texCvs.height = 64;
  ...
  const makeLabel = (node: FolderNode, color: number): THREE.CanvasTexture => {
    ...
    texCtx.fillStyle = `rgba(${r},${g},${b},0.25)`; texCtx.roundRect(0, 0, 512, 64, 4); texCtx.fill();
    texCtx.fillStyle = "#e6edf3";
    texCtx.font = node.children.length > 0 ? "bold 18px monospace" : "14px monospace";
    texCtx.fillText(node.name, 10, 26);
    texCtx.fillStyle = "#6e7681"; texCtx.font = "11px sans-serif";
    const info: string[] = [];
    if (node.fileCount > 0) info.push(`${node.fileCount} files`);
    if (node.functionCount > 0) info.push(`${node.functionCount} fn`);
    if (node.children.length > 0) info.push(`${node.children.length} sub`);
    texCtx.fillText(info.join("  "), 10, 46);
    const clone = document.createElement("canvas"); clone.width = 512; clone.height = 64;
    clone.getContext("2d")!.drawImage(texCvs, 0, 0);
    const tex = new THREE.CanvasTexture(clone); tex.needsUpdate = true; return tex;
  };
  ...
  for (const [id, node] of nodes) {
    ...
    if (node.children.length > 0) {
      ...
      const floorMat = new THREE.MeshStandardMaterial({
        color: new THREE.Color(`rgb(${Math.floor(r * 0.2)},${Math.floor(g * 0.2)},${Math.floor(b * 0.2)})`),
        transparent: true, opacity: 0.6, roughness: 0.9,
      });
      const floor = new THREE.Mesh(floorGeo, floorMat);
      floor.position.set(bx, box.y + FLOOR_H / 2, bz); floor.name = id;
      scene.add(floor); cardMeshes.push(floor); meshToNode.set(floor, node);
      const wallH = box.h;
      const wallMat = new THREE.MeshStandardMaterial({
        color: new THREE.Color(`rgb(${r},${g},${b})`), transparent: true, opacity: 0.08, roughness: 0.9, side: THREE.DoubleSide,
      });
      const lw = new THREE.Mesh(new THREE.BoxGeometry(WALL, wallH, box.d), wallMat); lw.position.set(box.x - cx, box.y + wallH / 2, bz); scene.add(lw);
      const rw = new THREE.Mesh(new THREE.BoxGeometry(WALL, wallH, box.d), wallMat.clone()); rw.position.set(box.x + box.w - cx, box.y + wallH / 2, bz); scene.add(rw);
      const fw = new THREE.Mesh(new THREE.BoxGeometry(box.w, wallH, WALL), wallMat.clone()); fw.position.set(bx, box.y + wallH / 2, box.z - cz); scene.add(fw);
      const bw = new THREE.Mesh(new THREE.BoxGeometry(box.w, wallH, WALL), wallMat.clone()); bw.position.set(bx, box.y + wallH / 2, box.z + box.d - cz); scene.add(bw);
      const wireGeo = new THREE.BoxGeometry(box.w, wallH, box.d);
      const wireEdges = new THREE.EdgesGeometry(wireGeo);
      const wireMat = new THREE.LineBasicMaterial({ color: new THREE.Color(`rgb(${r},${g},${b})`), transparent: true, opacity: 0.15 });
      const wire = new THREE.LineSegments(wireEdges, wireMat); wire.position.set(bx, box.y + wallH / 2, bz); scene.add(wire);
      ...
      const labelMat = new THREE.MeshBasicMaterial({ map: labelTex, transparent: true, opacity: 0.9, side: THREE.DoubleSide });
      const label = new THREE.Mesh(labelGeo, labelMat);
      label.position.set(box.x + labelW / 2 + PAD - cx, box.y + wallH - 0.1, box.z + LABEL_ZONE * 0.4 - cz);
      label.rotation.x = -Math.PI / 4; scene.add(label);
    } else {
      // Leaf node with LOD
      const farLeaf = new THREE.Group();
      const farGeo = new THREE.BoxGeometry(box.w, box.h, box.d);
      const farTex = makeLabel(node, color);
      const farMat = new THREE.MeshStandardMaterial({ map: farTex, transparent: true, opacity: 0.95, roughness: 0.7, metalness: 0.1 });
      const farMesh = new THREE.Mesh(farGeo, farMat); farMesh.name = id;
      farMesh.position.set(0, box.h / 2, 0);
      farLeaf.add(farMesh);
      cardMeshes.push(farMesh); meshToNode.set(farMesh, node);

      const nearLeaf = new THREE.Group();
      const floorGeo = new THREE.BoxGeometry(box.w, FLOOR_H, box.d);
      const floorMat = new THREE.MeshStandardMaterial({
        color: new THREE.Color(`rgb(${Math.floor(r * 0.15)},${Math.floor(g * 0.15)},${Math.floor(b * 0.15)})`),
        transparent: true, opacity: 0.4, roughness: 0.9,
      });
      const floor = new THREE.Mesh(floorGeo, floorMat); floor.name = id;
      nearLeaf.add(floor);
      cardMeshes.push(floor); meshToNode.set(floor, node);
      ...
      pageCvs.width = 256; pageCvs.height = 320;
      ...
      for (let fi = 0; fi < files.length; fi++) {
        ...
        pageCtx.lineWidth = 1;
        pageCtx.beginPath(); pageCtx.moveTo(28, 0); pageCtx.lineTo(28, 320); pageCtx.stroke();
        ...
        pageClone.width = 256; pageClone.height = 320;
        ...
        const pageFront = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ map: pageTex, transparent: true, opacity: 0.95, side: THREE.DoubleSide }),
        );
        pageGroup.add(pageFront);

        const pageEdge = new THREE.Mesh(
          new THREE.BoxGeometry(PAGE_W, PAGE_H, PAGE_THICK),
          new THREE.MeshStandardMaterial({ color: 0x21262d, transparent: true, opacity: 0.5, roughness: 0.9 }),
        );
  ...
  for (const edge of edges) {
    if (!boxes.has(edge.from) || !boxes.has(edge.to)) continue;
    const arr = edgesBySource.get(edge.from) ?? [];
    arr.push(edge.to); edgesBySource.set(edge.from, arr);
  }

  const addWire = (group: THREE.Group, path: THREE.Curve<THREE.Vector3>, radius: number, color: number, opacity: number) => {
    const geo = new THREE.TubeGeometry(path, 20, radius, 8, false);
    group.add(new THREE.Mesh(geo, new THREE.MeshStandardMaterial({
      color, transparent: true, opacity,
      roughness: 0.35, metalness: 0.5, emissive: color, emissiveIntensity: 0.2,
    })));
  };

  const addArrow = (group: THREE.Group, target: THREE.Vector3, curve: THREE.Curve<THREE.Vector3>, color: number) => {
    const tangent = curve.getTangentAt(1.0).normalize();
    const cone = new THREE.Mesh(
      new THREE.ConeGeometry(0.2, 0.55, 8),
      new THREE.MeshStandardMaterial({
        color, transparent: true, opacity: 0.9,
        roughness: 0.3, metalness: 0.4, emissive: color, emissiveIntensity: 0.25,
      }),
    );
  ...
  const makeArc = (from: THREE.Vector3, to: THREE.Vector3, heightFactor: number): THREE.QuadraticBezierCurve3 => {
    const dist = from.distanceTo(to);
    const h = Math.min(dist * heightFactor, 6) + 0.8;
    const mid = new THREE.Vector3((from.x + to.x) / 2, Math.max(from.y, to.y) + h, (from.z + to.z) / 2);
  ...
  for (const [fromId, targets] of edgesBySource) {
    ...
    const origin = new THREE.Vector3(fb.x + fb.w / 2 - cx, anchorY(fb, fromId), fb.z + fb.d / 2 - cz);
    ...
    for (const toId of targets) {
      ...
      const local = rel(new THREE.Vector3(tb.x + tb.w / 2 - cx, anchorY(tb, toId), tb.z + tb.d / 2 - cz));
      targetLocals.push({ toId, pt: local });
      centX += local.x; centY += local.y; centZ += local.z;
    }
    centX /= targets.length; centY /= targets.length; centZ /= targets.length;
    ...
    for (const { toId, pt } of targetLocals) {
      ...
      addWire(nearGroup, curve, 0.025, fromColor, 0.7);
      addArrow(nearGroup, pt, curve, toColor);
    }

    const midGroup = new THREE.Group();
    for (const { pt } of targetLocals) {
      const curve = makeArc(srcLocal, pt, 0.3);
      addWire(midGroup, curve, 0.015, fromColor, 0.45);
    }

    const farGroup = new THREE.Group();
    const farCurve = makeArc(srcLocal, centroidLocal, 0.25);
    addWire(farGroup, farCurve, 0.04, fromColor, 0.55);
