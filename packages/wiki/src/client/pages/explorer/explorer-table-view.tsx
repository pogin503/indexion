...
export const ExplorerTableView = ({ functions, filter, onSelectFn }: Props): React.JSX.Element => {
      ...
      <div className="min-w-[640px]">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-border">
            {data.map((fn) => (
              <tr key={fn.id} className="cursor-pointer transition-colors hover:bg-muted/50" onClick={() => onSelectFn?.(fn)}>
                <td className="whitespace-nowrap px-4 py-1.5 font-mono text-xs">{fn.name}</td>
                <td className={`whitespace-nowrap px-4 py-1.5 text-xs ${kindTextClass(fn.kind)}`}>{fn.kind}</td>
                <td className="whitespace-nowrap px-4 py-1.5 font-mono text-xs text-muted-foreground">{fn.module}</td>
                <td className="px-4 py-1.5 text-center text-xs tabular-nums">{fn.depth}</td>
                <td className="px-4 py-1.5 text-center text-xs tabular-nums">{fn.callers.length || ""}</td>
                <td className="px-4 py-1.5 text-center text-xs tabular-nums">{fn.callees.length || ""}</td>
