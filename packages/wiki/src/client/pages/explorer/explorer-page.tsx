...
export const ExplorerPage = (): React.JSX.Element => {
  ...
  const graphState = useCachedApiCall<CodeGraph>(CacheKey.digest.graph, () => fetchGraph(client));
  const indexState = useCachedApiCall<ReadonlyArray<IndexedFunction>>(CacheKey.digest.index, () => fetchDigestIndex(client));
  const [selectedSym, setSelectedSym] = useState<SymEntry | null>(null);
  const [selectedFnDirect, setSelectedFnDirect] = useState<IndexedFunction | null>(null);
  ...
  const selectedFn = useMemo(() => {
    if (selectedFnDirect) return selectedFnDirect;
  ...
  const handleSelectSym = useCallback((sym: SymEntry) => { setSelectedFnDirect(null); setSelectedSym(sym); }, []);
  const handleSelectFn = useCallback((fn: IndexedFunction) => { setSelectedSym(null); setSelectedFnDirect(fn); }, []);
  ...
  const totalSymbols = indexState.status === "success" ? indexState.data.length : 0;
  ...
  return (
    <div className="grid h-full grid-rows-[auto_1fr]">
      <ExplorerToolbar
        ...
        stats={{ roots: tree?.length ?? 0, files: totalFiles, symbols: totalSymbols }}
      />

      <div className="relative grid min-h-0 grid-cols-[1fr] md:data-[detail]:grid-cols-[1fr_360px]" data-detail={selectedFn ? "" : undefined}>
        {view === "tree" && (
          <ExplorerTreeView tree={filteredTree ?? []} onSelectSym={handleSelectSym} />
        )}
        {view === "table" && (
          <ExplorerTableView
        ...
        {selectedFn && (
          <ScrollArea className="absolute inset-0 z-10 h-full border-l bg-background md:static md:inset-auto md:z-auto">
            <FunctionDetail fn={selectedFn} onClose={() => { setSelectedSym(null); setSelectedFnDirect(null); }} />
          </ScrollArea>
        )}
      </div>
    </div>
