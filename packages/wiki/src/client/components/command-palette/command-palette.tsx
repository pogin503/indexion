...
import { fetchGraph, queryDigest, searchWiki, type CodeGraph, type DigestMatch } from "@indexion/api-client";
...
export const CommandPalette = ({ open, onOpenChange }: Props): React.JSX.Element => {
  ...
  const [semanticResults, setSemanticResults] = useState<ReadonlyArray<DigestMatch>>([]);
  const debounceRef = useRef(0);

  // Load graph once for symbol search (explorer scope), via app-level cache
  useEffect(() => {
    if (!open || graph || scope !== "explorer") return;
    cachedFetch(CacheKey.digest.graph, () => fetchGraph(client)).then((r) => { if (r.ok) setGraph(r.data); });
  }, [open, graph, scope]);

  // Debounced server searches — scoped
  useEffect(() => {
    if (!query.trim()) {
      setWikiResults([]);
      setSemanticResults([]);
    ...
    const timeout = setTimeout(async () => {
      if (debounceRef.current !== id) return;

      if (scope === "wiki") {
        // Wiki scope: only search wiki
        const wiki = await searchWiki(client, { query: query.trim(), topK: 10 });
        if (debounceRef.current !== id) return;
        if (wiki.ok) setWikiResults(wiki.data as ReadonlyArray<WikiSearchHit>);
      } else {
        // Explorer scope: symbol (client-side) + semantic (server)
        const semantic = await queryDigest(client, { purpose: query.trim(), topK: 10 });
        if (debounceRef.current !== id) return;
        if (semantic.ok) setSemanticResults(semantic.data);
      }
    }, DEBOUNCE_MS);
  ...
  // Reset on close
  useEffect(() => {
    if (!open) { setQuery(""); setWikiResults([]); setSemanticResults([]); }
  }, [open]);
  ...
  const symbolResults = (() => {
    if (scope !== "explorer" || !graph || !query.trim()) return [];
  ...
  const hasResults = symbolResults.length > 0 || wikiResults.length > 0 || semanticResults.length > 0;
      ...
      <div className="fixed left-1/2 top-[15%] z-50 w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 overflow-hidden rounded-lg border bg-background shadow-2xl">
        <div className="flex items-center gap-2 border-b px-3">
          ...
          <Command.Input
            ...
            placeholder={scope === "wiki" ? "Search wiki pages..." : "Search symbols or by purpose..."}
        ...
        <Command.List className="max-h-80 overflow-y-auto p-1">
          ...
          {wikiResults.length > 0 && (
            <Command.Group heading="Wiki Pages">
              <div className="px-1 py-1 text-xs font-semibold text-muted-foreground">Wiki Pages</div>
              {wikiResults.map((hit, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate font-medium">{hit.section.title || hit.section.page_id}</div>
                    <div className="truncate text-xs text-muted-foreground">{hit.section.page_id}</div>
                  </div>
          ...
          {symbolResults.length > 0 && (
            <Command.Group heading="Symbols">
              <div className="px-1 py-1 text-xs font-semibold text-muted-foreground">Symbols</div>
              {symbolResults.map((sym) => (
                <Command.Item
                  key={sym.id}
                  value={`sym-${sym.id}`}
                  onSelect={() => go(`/?filter=${encodeURIComponent(sym.name)}`)}
                  className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm text-foreground aria-selected:bg-accent"
                >
                  <FileCode2 className="size-3.5 shrink-0 text-blue-400" />
                  <span className="truncate font-mono">{sym.name}</span>
                  <span className="ml-auto shrink-0 truncate text-xs text-muted-foreground">{sym.module}</span>
                </Command.Item>
              ))}
            </Command.Group>
          )}
          ...
          {semanticResults.length > 0 && (
            <Command.Group heading="By Purpose">
              <div className="px-1 py-1 text-xs font-semibold text-muted-foreground">By Purpose</div>
              {semanticResults.map((r, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-muted-foreground">{r.file}</div>
                  </div>
                  <span className="shrink-0 text-[10px] tabular-nums text-muted-foreground">
