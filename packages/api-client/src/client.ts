...
export type HttpClient = {
  readonly get: <T>(path: string, signal?: AbortSignal) => Promise<ApiResponse<T>>;
  readonly post: <T>(path: string, body: unknown, signal?: AbortSignal) => Promise<ApiResponse<T>>;
};

/** GET request. */
const doGet = async <T>(baseUrl: string, path: string, signal?: AbortSignal): Promise<ApiResponse<T>> => {
  try {
...
/** POST request. */
const doPost = async <T>(baseUrl: string, path: string, body: unknown, signal?: AbortSignal): Promise<ApiResponse<T>> => {
  try {
...
export const createHttpClient = (baseUrl: string): HttpClient => ({
  get: <T>(path: string, signal?: AbortSignal) => doGet<T>(baseUrl, path, signal),
  post: <T>(path: string, body: unknown, signal?: AbortSignal) => doPost<T>(baseUrl, path, body, signal),
});
