...
export const FolderItem = ({ folder, depth, onSelectSym }: Props): React.JSX.Element => {
      ...
      <CollapsibleTrigger asChild>
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-1.5">
            {totalSyms > 0 && <Badge variant="secondary" className="text-[10px]">{totalSyms} sym</Badge>}
            <Badge variant="outline" className="text-[10px]">{folder.files.length} files</Badge>
          </div>
        </button>
      </CollapsibleTrigger>
      <CollapsibleContent>
        {folder.children.map((child) => (
          <FolderItem key={child.path} folder={child} depth={depth + 1} onSelectSym={onSelectSym} />
        ))}
        {folder.files.map((file) => (
          <FileItem key={file.path} file={file} depth={depth + 1} onSelectSym={onSelectSym} />
        ))}
      </CollapsibleContent>
    </Collapsible>
  );
